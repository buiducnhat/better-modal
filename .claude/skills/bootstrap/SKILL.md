...
- **Upstream**: None. `bootstrap` is the entry point for new projects.
- **Downstream**:
  - `docs --update` — refresh documentation after the project evolves
  - `brainstorm` — explore architecture or design questions before planning
  - `write-plan` — create the first implementation plan for a feature
  - `execute-plan` — execute an approved plan
  - `quick-implement` — implement a small, well-defined task immediately
